...
Sprite 랜더링을 위해 삼각형 두개를 합쳐 사각형을 만들어야한다.

[코드]

그리고 각 삼각형에 맵핑되는텍스처의 UV 좌표와 인덱스 버퍼와 함께 넘겨주면 Sprite 랜더링을 할 수 있다.

[코드]

### 텍스처 아틀라스

Primitive 타입 지정을 통해 떨어진 위치에 있는 사각형, 즉 Sprite를 화면에 랜더링할 수 있다. 하지만 각 스프라이트가 다른 텍스처를 사용한다면 Batch 처리를 할 수 없다. Batch 처리를 위해 버텍스, UV 그리고 인덱스를 한번에 전달해야 하는데, UV는 